...
- 查询物流状态支持多种查询方式：运单号、订单号(systemnumber)、客户参考号（即 `query_order_status(order_number)`）。
...
```text
请立刻调用 submit_forecast_order_from_text，并只返回工具 JSON（不要总结）：text="从深圳到洛杉矶；customernumber1=T620200611-1001；收件国家=US；收件人=John；收件地址=123 Main St；城市=Los Angeles；邮编=90001；省州=CA；投保=是；保额=100；险种=货物运输险；币别=人民币；报关类型=买单报关；物品类别=普货"
```

**重要提示：报关类型选择**
- ✅ 明确指定：`报关类型=买单报关` 或 `报关类型=贸易报关`
- ✅ 不需报关：`报关类型=不需报关`
- ❌ 避免模糊：`报关类型=需要报关` 会提示选择具体类型
...
**方式2：订单号查询**
```text
请立刻调用 query_order_status("17169126467343")，并只返回工具 JSON（不要总结）：
```

**方式3：客户参考号查询**
```text
请立刻调用 query_order_status("T620200611-1001")，并只返回工具 JSON（不要总结）：
```
...
text="从深圳到洛杉矶；customernumber1=T620200611-1001；收件国家=US；收件人=John；收件地址=123 Main St；城市=Los Angeles；邮编=90001；省州=CA；投保=是；保额=100；险种=货物运输险；币别=USD；物品类别=普货；报关类型=买单报关"
```
...
# 支持多种查询方式：
查询订单 #12345 的物流状态                    # 演示订单
查询订单 EV11396275052CN 的物流状态          # 运单号查询
查询订单 18564645902272 的物流状态           # 订单号(systemnumber)查询  
查询订单 T6241214001 的物流状态              # 客户参考号查询

# 或直接调用工具：
请调用 query_order_status("12345")          # 支持运单号、订单号、客户参考号
请调用 query_order_status("EV11396275052CN")  # 运单号
请调用 query_order_status("18564645902272")   # 订单号
请调用 query_order_status("T6241214001")      # 客户参考号
```

### 示例2：创建新货运单
```bash
# 自然语言方式：
请调用 submit_forecast_order_from_text，text="从深圳到洛杉矶；customernumber1=T620200611-1001；收件国家=US；收件人=John；收件地址=123 Main St；城市=Los Angeles；邮编=90001；省州=CA；投保=是；保额=100；险种=货物运输险；币别=USD；物品类别=普货；报关类型=买单报关"
...
### 🟢 明确输入方式（推荐）
- `报关类型=买单报关` (code: 2) - 适用于一般贸易
- `报关类型=贸易报关` (code: 3) - 适用于正式贸易报关
- `报关类型=不需报关` (code: 1) - 无需报关的货物
...
系统会提示：**"报关类型不明确，请选择具体类型：买单报关, 贸易报关"**

### 💡 设计理念
- **用户自主选择**：不自动映射到默认类型，让用户明确选择最适合的报关方式
- **清晰提示**：当输入不明确时，提供具体的选择选项和示例
- **业务准确性**：确保报关类型选择符合实际业务需求

## 重要说明

### 订单号和运单号显示

系统会为每个成功创建的订单返回两个重要的标识符：

- **订单号 (Order Number/系统单号)**：用于内部订单管理和查询
- **运单号 (Tracking Number)**：用于物流跟踪和快递查询

**Agent 已配置为同时显示这两个号码**。如果在使用过程中只看到运单号，可以：

1. 明确要求显示订单号：`请显示订单号和运单号`
2. 使用工具查询：`请调用 get_last_order_reference`
3. 重新创建订单并要求完整信息
...
```bash
# 测试订单创建和号码提取
python test_final_order_display.py

# 测试缺失字段验证
python test_missing_countrycode.py
